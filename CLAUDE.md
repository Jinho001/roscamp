...
### 1. Plan Node Default
- 3단계 이상이거나 아키텍처 결정이 필요한 **모든 비자명 작업**은 플랜 모드 진입
- 작업이 예상과 다르게 흘러가면 **즉시 중단하고 재플랜** — 계속 밀어붙이지 않는다
- 플랜 모드는 구현뿐 아니라 검증 단계 설계에도 활용
- 모호함을 줄이기 위해 사전에 상세 스펙을 작성한다

### 2. Subagent Strategy
- 메인 컨텍스트 윈도우를 깔끔하게 유지하기 위해 서브에이전트를 적극 활용
- 연구, 탐색, 병렬 분석은 서브에이전트에 위임
- 복잡한 문제일수록 서브에이전트를 통해 더 많은 컴퓨팅을 투입
- 서브에이전트 1개당 1가지 목표에 집중

### 3. Self-Improvement Loop
- 사용자에게 **수정을 받은 후**: `tasks/lessons.md`에 패턴 기록
- 같은 실수를 막는 규칙을 직접 작성한다
- 실수율이 떨어질 때까지 교훈을 반복 개선
- 세션 시작 시 관련 프로젝트의 교훈을 검토

### 4. Verification Before Done
- 작동을 증명하지 않은 상태로 작업 완료 표시 금지
- 필요 시 main 대비 변경사항의 동작 차이를 diff로 확인
- "시니어 엔지니어가 이 코드를 승인하겠는가?" 자문
- 테스트 실행, 로그 확인, 정확성 시연

### 5. Demand Elegance (Balanced)
- 비자명 변경 시: "더 우아한 방법이 있는가?" 잠깐 멈춰 생각
- 수정이 핵처럼 느껴진다면: "지금 아는 모든 걸 바탕으로 우아한 해결책을 구현하라"
- 단순하고 명백한 수정은 이 단계를 건너뜀 — 과도한 설계 금지
- 제출 전에 스스로의 작업에 의문을 제기

### 6. Autonomous Bug Fixing
- 버그 리포트를 받으면: 그냥 고친다. 손잡아 달라고 하지 않는다
- 로그, 에러, 실패 테스트를 지목하고 해결
- 사용자의 컨텍스트 전환 없이 처리
- 지시 없이도 실패하는 CI 테스트를 수정

---

## Task Management

1. **Plan First**: `tasks/todo.md`에 체크 가능한 항목으로 플랜 작성
2. **Verify Plan**: 구현 시작 전 플랜 확인
3. **Track Progress**: 진행하면서 항목 완료 표시
4. **Explain Changes**: 각 단계에서 고수준 요약 제공
...
- **Simplicity First**: 모든 변경을 최대한 단순하게. 코드 영향 범위 최소화.
- **No Laziness**: 근본 원인을 찾는다. 임시방편 금지. 시니어 개발자 기준.
- **Minimal Impact**: 꼭 필요한 부분만 수정. 사이드 이펙트 최소화.
