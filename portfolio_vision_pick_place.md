...
> **한 줄 요약:** USB 카메라 + YOLO OBB 검출 + 핀홀 역산으로 로봇 팔이 임의 위치의 상자를 자율적으로 파지하는 파이프라인을 설계하고, 단계별 검증 도구로 정확도를 측정·보정해 **10회 연속 100% 파지 성공**을 달성했습니다.

---

## 배경

무신사(Moosinsa) 프로젝트는 ROS 2 기반 자율주행 로봇(AMR)과 로봇 팔이 협력해 고객이 선택한 시착품을 자동 전달하는 시스템입니다.
저는 **FrontJet(ARM 로봇)의 비전 기반 Pick & Place 파이프라인** 전담을 맡았습니다.
로봇이 테이블 위 임의 위치의 신발 상자를 카메라로 인식하고 자율적으로 집어드는 기능입니다.
...
```mermaid
graph TD
    A["USB 카메라<br/>(Eye-in-Hand, flange 장착)"]
    B["cv_detect_server<br/>HSV OBB 검출 서버<br/>cx, cy, w, h, theta 반환"]
    C["CoordTransformer<br/>픽셀 → base_link 3D 좌표<br/>Ray-Plane 교점 알고리즘"]
    D["MotionController<br/>pymycobot MyCobot280 제어<br/>Approach → Grasp → Retreat"]
    E["FMS (Fleet Management System)<br/>ROS2 Action Server<br/>AMR 협업 트리거"]
...
**효과:** `python3 scripts/verify.py` 한 줄로 ROS2 빌드 없이 파이프라인 전체를 검증할 수 있게 되었습니다. 개발·디버깅 사이클이 크게 단축됐습니다.
...
graph LR
    P["픽셀 (cx, cy)"]
    K["카메라 내부 파라미터 K<br/>(캘리브레이션)"]
    T1["T_flange2cam<br/>(Hand-Eye 캘리브)"]
    T2["T_base2flange<br/>(FK + 현재 관절각)"]
    T3["T_base2cam<br/>(= T_base2flange @ T_flange2cam)"]
    R["Ray-Plane 교점<br/>z = z_surface_mm"]
    X["base_link 3D 좌표 (mm)"]
...
### 버그 1. TCP / Flange 좌표계 혼재 → 110mm 오차
...
```
TCP 좌표:    [-58.5, -181.6, 154.5 mm]  ← observe_pose 이동 명령용
flange 좌표: [-60.5, -165.0, 265.0 mm]  ← T_base2cam 계산에 사용해야 할 값
                                  ↑
                             z 기준 110mm 차이
```
...
### 버그 2. z_surface_mm 오설정 → 픽셀 스케일 역산 오류
...
```
flange_z = 129.3 mm,  tcp_offset_z = 110 mm
→ z_surface = 129.3 − 110 = 19.3 mm  ≈  25.5 mm  (Step A 검증으로 정밀화)
```

---

### 버그 3. 실측 좌표계 불일치 → 기준값 오염
...
```
OBB 검출:    w=159 px  h=115 px
카메라 높이: 235.6 mm  |  d_z: 210.1 mm
추정 크기:   33.72 mm × 24.98 mm
실제 크기:   33.0  mm × 25.0  mm
오차:        가로 +0.72 mm / 세로 −0.02 mm  ✅
```
...
```
변환 결과: x=−48.25 mm   y=−217.17 mm
실측 위치: x=−47.3  mm   y=−200.9  mm
오차:      Δx=−0.95 mm   Δy=−16.27 mm   총=16.30 mm
```
...
실제 카메라 물리적 위치: y≈−50 mm 내외 (자로 실측)
→ 약 11 mm 차이 + 추가 오차
```
...
**8/8 성공 (100%)** — 이후 자동화 검증 스크립트로 10회 연속 100% 추가 달성

### 시스템 응답 성능

| 지표 | 측정값 |
|------|--------|
| 비전 파이프라인 (캡처 → 3D 좌표 변환) | 평균 **74.8 ms** (최대 384.6 ms) |
| Pick & Place 전체 사이클 (Approach → Grasp → Retreat) | 평균 **12.62 초** |

> 평균 74.8 ms의 비전 처리 속도는 AMR과의 협업 시 명령 지연을 최소화하며,
> 실시간 로봇 제어 시스템으로서의 요건을 충족합니다.
...
| 항목 | 현황 | 개선 방향 |
|------|------|---------|
| 각도 대응 범위 | ±20도 내에서만 안정적 | 핸드아이 재캘리브 (번역벡터 정밀화) |
| y축 오차 | pick_offset으로 임시 보정 | 재캘리브로 근본 해결 |
| 상자 인식 방식 | HSV 기반 (흰 상자) | YOLO OBB로 확장 → 다양한 색상 대응 |
...
| 파일 | 역할 |
|------|------|
| `core/coord_transform.py` | 픽셀 → 3D 좌표 변환 |
| `core/motion.py` | Pick/Place 모션 제어 |
| `scripts/verify.py` | 단계별 독립 검증 도구 |
