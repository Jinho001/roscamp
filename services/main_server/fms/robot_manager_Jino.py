...
class RobotManager:
    ...
    def __init__(self):
        ...
        # ── Jetcobot 토픽 연동 — pick/place/reset result 대기 이벤트 ─────────
        self._jetcobot_pick_event  = threading.Event()
        self._jetcobot_place_event = threading.Event()
        self._jetcobot_reset_event = threading.Event()
        self._jetcobot_pick_result:  dict = {}
        self._jetcobot_place_result: dict = {}
    ...
    def connect_all(self):
        ...
        try:
            ...
            # Jetcobot pick/place/reset result 토픽 구독
            self._sub(self._client, "/jetcobot/result/pick",
                      "std_msgs/String", self._on_jetcobot_pick_result)
            self._sub(self._client, "/jetcobot/result/place",
                      "std_msgs/String", self._on_jetcobot_place_result)
            self._sub(self._client, "/jetcobot/result/reset",
                      "std_msgs/String", self._on_jetcobot_reset_result)
            print("[fleet] jetcobot result 토픽 구독 완료")
        ...
        if s == TRYON_STAGE_TO_WAREJET:
            ...
            def _run_warejet_then_advance():
                ...
                if state.tryon_stage == TRYON_STAGE_AT_WAREJET:  # 중간 cancel 체크
                    ...
                    self._post_delivery_status(state,"쇼피가 시착존으로 이동중 입니다.")
                else:
    ...
    def _post_delivery_status(self, state: _RobotState, status: str):
        ...
        try:
            response = requests.post(url, json={"robot_id": state.robot_id, "type": "AMR_DELIVERY_STATUS", "status": status}, timeout=5.0)
    ...
    def _do_warejet_store_inbound(self, task: InboundTask):
        ...
        ok = self._jetcobot_place("warehouse_place")
    ...
    def _do_frontjet_load_retrieval(self, task: RetrievalTask):
        ...
        ok = self._jetcobot_pick("receiving_zone")
    ...
    def arm_reset(self, robot_id: str) -> bool:
        return self._jetcobot_reset(robot_id)
    ...
    def _on_jetcobot_pick_result(self, msg) -> None:
        try:
            import json
            self._jetcobot_pick_result = json.loads(msg["data"])
        except Exception:
            self._jetcobot_pick_result = {"success": False}
        self._jetcobot_pick_event.set()

    def _on_jetcobot_place_result(self, msg) -> None:
        try:
            import json
            self._jetcobot_place_result = json.loads(msg["data"])
        except Exception:
            self._jetcobot_place_result = {"success": False}
        self._jetcobot_place_event.set()

    def _on_jetcobot_reset_result(self, msg) -> None:
        self._jetcobot_reset_event.set()

    def _jetcobot_pick(self, location: str, box_index: int = -1,
                       timeout: float = 60.0) -> bool:
        """roslibpy로 /jetcobot/cmd/pick 발행 후 result 대기. 블로킹."""
        import json
        if not self._client or not self._client.is_connected:
            print("[fleet] jetcobot pick 실패: rosbridge 미연결")
            return False
        pub = self._get_pub("front_jet", "/jetcobot/cmd/pick",
                            "std_msgs/String", self._client)
        self._jetcobot_pick_event.clear()
        pub.publish(roslibpy.Message({
            "data": json.dumps({"location": location, "box_index": box_index})
        }))
        print(f"[fleet] jetcobot pick 발행: location={location}")
        ok = self._jetcobot_pick_event.wait(timeout=timeout)
        if not ok:
            print(f"[fleet] jetcobot pick 타임아웃 (location={location})")
            return False
        success = self._jetcobot_pick_result.get("success", False)
        print(f"[fleet] jetcobot pick result: success={success} "
              f"msg={self._jetcobot_pick_result.get('message', '')}")
        return success

    def _jetcobot_place(self, location: str, box_index: int = -1,
                        timeout: float = 60.0) -> bool:
        """roslibpy로 /jetcobot/cmd/place 발행 후 result 대기. 블로킹."""
        import json
        if not self._client or not self._client.is_connected:
            print("[fleet] jetcobot place 실패: rosbridge 미연결")
            return False
        pub = self._get_pub("ware_jet", "/jetcobot/cmd/place",
                            "std_msgs/String", self._client)
        self._jetcobot_place_event.clear()
        pub.publish(roslibpy.Message({
            "data": json.dumps({"location": location, "box_index": box_index})
        }))
        print(f"[fleet] jetcobot place 발행: location={location}")
        ok = self._jetcobot_place_event.wait(timeout=timeout)
        if not ok:
            print(f"[fleet] jetcobot place 타임아웃 (location={location})")
            return False
        success = self._jetcobot_place_result.get("success", False)
        print(f"[fleet] jetcobot place result: success={success} "
              f"msg={self._jetcobot_place_result.get('message', '')}")
        return success

    def _jetcobot_reset(self, robot_id: str, timeout: float = 30.0) -> bool:
        """roslibpy로 /jetcobot/cmd/reset 발행 후 result 대기. 블로킹."""
        import json
        if not self._client or not self._client.is_connected:
            print(f"[fleet] jetcobot reset 실패: rosbridge 미연결")
            return False
        pub = self._get_pub(robot_id, "/jetcobot/cmd/reset",
                            "std_msgs/String", self._client)
        self._jetcobot_reset_event.clear()
        pub.publish(roslibpy.Message({"data": json.dumps({})}))
        print(f"[fleet] jetcobot reset 발행: robot_id={robot_id}")
        ok = self._jetcobot_reset_event.wait(timeout=timeout)
        if not ok:
            print(f"[fleet] jetcobot reset 타임아웃")
        return ok
