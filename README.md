...
| 파지 좌표 평균 오차 | **< 0.95 mm** | `verify.py` Step B: 실측 위치 vs 추정 위치 | < 5.0 mm |
| 비전 3D 변환 지연 | 평균 **74.8 ms** (최대 384.6 ms) | `--measure-cycle` 모드 10회 반복 | — |
| Pick & Place 사이클 타임 | 평균 **12.62 s** | `--measure-cycle` 모드 10회 반복 | — |
| 초기 파지 검증 | **8회 연속 성공** (n=8) | 임의 위치 배치 8회 연속 실파지 테스트 | — |

> **최대 지연 384.6 ms 주석**: 평균 74.8 ms 대비 5배 이상인 이 수치는 UDP 패킷 손실 후 cv_detect_server 폴링 재시도 또는 검출 서버 부하 피크에서 발생합니다. 사이클 타임에는 영향이 없지만, 이 편차가 **C++ 포팅의 우선순위를 결정한 근거**입니다.
...
|------|---------|---------|---------|
| **C++ 포팅** | Python NumPy: 평균 74.8 ms, 최대 384.6 ms | `core/` → Eigen + OpenCV C++ | 레이턴시 **20 ms 이내** (70% 단축 예상) |
| **ROS2 Lifecycle Node** | 게이팅이 순차 코드 흐름에 의존 | `inactive` → `active` 표준 상태 머신 | 재시작 안전성, 상태 명시성 확보 |
| **Visual Servoing** | 개루프: 관측 후 1회 좌표 계산 | 카메라 피드백 기반 폐루프 제어 | 외부 섭동·조립 공차에 실시간 대응 |
