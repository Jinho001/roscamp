...
| 항목 | 내용 |
| :--- | :--- |
| **개발 기간** | 9주 (2026.03.16 – 2026.05.18) |
...
| **파지 정밀도** | 설계 목표 **Δx ≤ 5 mm** | PointCloud 노이즈 약 2~5 mm (근거리) |
...
|---|---|---|
| **측정 지연** | **40.4 ms** (평균, 초기 로딩 이상치 제외) | **> 150 ms** (DDS 직렬화 오버헤드 실측) |
...
| | **채택: Core/Node 분리 (v2)** | 탈락: ROS2 단일 노드 구조 (v1) |
|---|---|---|
...
| **결과** | Y축 오차 **16.3 mm → 허용 기준(≤5 mm) 이내** |
